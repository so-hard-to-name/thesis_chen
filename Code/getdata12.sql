...
, vitalsign AS (
    ...
    LEFT JOIN `physionet-data.mimiciv_derived.vitalsign` ce
        ON ie.stay_id = ce.stay_id
            ...
            AND ce.charttime <= DATETIME_ADD(ie.intime, INTERVAL '12' HOUR)
...
, gcs AS (
    ...
    LEFT JOIN `physionet-data.mimiciv_derived.gcs` g
        ON ie.stay_id = g.stay_id
            ...
            AND g.charttime <= DATETIME_ADD(ie.intime, INTERVAL '12' HOUR)
)
...
, urinefinal AS (
    ...
    LEFT JOIN `physionet-data.mimiciv_derived.urine_output` uo
        ON ie.stay_id = uo.stay_id
            ...
            AND uo.charttime <= DATETIME_ADD(ie.intime, INTERVAL '12' HOUR)
    GROUP BY ie.subject_id, ie.stay_id
)
...
SELECT 
    ...
    JOIN vs_final
        ON demographic.stay_id = vs_final.stay_id
            ...
            AND vs_final.starttime <= DATETIME_ADD(demographic.intime, INTERVAL '12' HOUR)
